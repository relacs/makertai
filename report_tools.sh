...
function performance_header {
    ...
    N=$($DIR/../makertaikernel.sh report -f dat --select kern_latencies:n -u $FILE | grep -v '^#')
    N=$(echo $N)
...
function performance_data {
    ...
    echo "### $TITLE"
    echo
    $DIR/../makertaikernel.sh report -f md ${SHOW_COLUMNS[@]/#/--select } -s "$SORTCOL" -g $DIR/$PLOTFILE -u -m 'none' $@ | sed -e 's/ jitter//'
    echo
